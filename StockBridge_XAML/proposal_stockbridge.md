...
A.	Latar Belakang
Perkembangan teknologi informasi dan komunikasi saat ini telah membawa perubahan besar pada sistem manajemen operasional di berbagai sektor industri, termasuk dalam bidang manajemen logistik dan pergudangan. Efisiensi, kecepatan, dan akurasi data dalam pelacakan persediaan barang (inventaris) kini menjadi kunci utama bagi keberhasilan operasional perusahaan.

Studi kasus dalam proyek ini diambil pada **PT. Sentosa Abadi Logistik**, sebuah perusahaan distributor berskala menengah yang mendistribusikan kebutuhan Alat Tulis Kantor (ATK), peralatan elektronik, dan aset IT ke berbagai divisi internal maupun kantor cabang. Saat ini, PT. Sentosa Abadi Logistik masih menghadapi kendala besar dalam pengelolaan gudang mereka karena masih mengandalkan pencatatan manual berbasis spreadsheet (Microsoft Excel) dan buku besar fisik. 

Metode pencatatan manual tersebut menimbulkan berbagai kelemahan operasional yang signifikan di PT. Sentosa Abadi Logistik, di antaranya:
1. **Ketidakakuratan Data Stok (Human Error):** Sering terjadi selisih antara jumlah barang fisik di rak gudang dengan catatan digital karena kelalaian petugas dalam memperbarui data.
2. **Kekacauan Konversi Satuan (Unit of Measurement):** Barang sering kali dibeli dalam jumlah besar seperti karton/dus (Pack), namun didistribusikan dalam bentuk eceran seperti pcs/buah (Unit). Perhitungan konversi secara manual rentan terhadap kesalahan hitung.
3. **Lemahnya Sistem Pengawasan (Lack of Audit Trail):** Barang gudang sering keluar-masuk tanpa ada otorisasi tertulis, sehingga tidak diketahui siapa penanggung jawab transaksi tersebut apabila terjadi kehilangan barang.
4. **Lambatnya Proses Stok Opname:** Pencarian dan verifikasi barang memakan waktu lama karena staf harus mencocokkan dokumen fisik satu per satu secara manual ke seluruh area gudang.

Guna mengatasi seluruh permasalahan di PT. Sentosa Abadi Logistik tersebut, dibutuhkan sebuah sistem manajemen logistik terintegrasi yang mampu mengotomatisasi pencatatan stok, menjamin keamanan data lewat pembatasan hak akses (Role-Based Access Control), mempermudah konversi satuan secara otomatis, serta mempercepat proses verifikasi barang. Oleh karena itu, penulis merancang dan membangun sebuah aplikasi manajemen logistik berbasis desktop yang diberi nama **StockBridge**. Aplikasi ini dibangun menggunakan bahasa pemrograman C# dengan framework Windows Presentation Foundation (WPF) untuk menghasilkan antarmuka pengguna yang modern, serta didukung oleh database SQL Server LocalDB dan micro-ORM Dapper untuk memastikan performa pengolahan data yang cepat dan andal.

Selain fitur manajemen data stok standar, StockBridge juga mengintegrasikan teknologi QR Code menggunakan library ZXing.Net untuk mempermudah identifikasi barang secara nirkabel di gudang PT. Sentosa Abadi Logistik. Aplikasi ini dilengkapi dengan asynchronous HTTP server listener pada port 8080, yang memungkinkan perangkat smartphone di jaringan lokal yang sama bertindak sebagai pemindai barcode nirkabel secara real-time. Melalui solusi terintegrasi ini, diharapkan operasional logistik barang di PT. Sentosa Abadi Logistik dapat berjalan dengan lebih efisien, terstruktur, aman, dan meminimalkan kesalahan operasional.
